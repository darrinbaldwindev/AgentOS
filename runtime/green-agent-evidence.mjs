...
/** Read-only normalization of existing AgentOS execution/audit/CI evidence for Green Agent consumption. */
export function normalizeGreenEvidence({ packet, now = Date.now() } = {}) {
  ...
  const taskTraceCounts = new Map();
  for (const task of tasks) if (task?.wake_trace_id) taskTraceCounts.set(task.wake_trace_id, (taskTraceCounts.get(task.wake_trace_id) ?? 0) + 1);
  const duplicateTraces = [...taskTraceCounts.entries()].filter(([, count]) => count > 1).map(([trace]) => trace);
  const traces = new Set([...tasks, ...responses, ...audit, ...expectedWakes].map((item) => item?.wake_trace_id).filter(Boolean));
  const mismatches = [];
  const evidence = [];
  for (const run of workflowRuns) {
    const runCommit = run?.head_sha ?? run?.commit_sha;
    if (runCommit && runCommit !== packet.commit_sha) mismatches.push({ type: 'WORKFLOW_COMMIT_MISMATCH', workflow_run_id: run.id ?? null, expected: packet.commit_sha, actual: runCommit });
  ...
  if (tasks.some((task) => task?.status === 'completed' && !task?.verification && !task?.evidence)) mismatches.push({ type: 'CONTRADICTORY_COMPLETION_EVIDENCE' });
  const redTypes = new Set(['CONTRADICTORY_COMPLETION_EVIDENCE', 'WORKFLOW_COMMIT_MISMATCH', 'AUDIT_COMMIT_MISMATCH', 'CLOCK_DRIFT_BREACH']);
  return Object.freeze({ schema: 'agentos-green-evidence-v0.1', project: packet.project, repository: packet.repository, commit_sha: packet.commit_sha, normalized_at: new Date(now).toISOString(), read_only: true, evidence, identity: Object.freeze({ task_count: tasks.length, response_count: responses.length, audit_count: audit.length, workflow_run_count: workflowRuns.length, expected_wake_count: expectedWakes.length }), traces: Object.freeze({ trace_count: traces.size, duplicate_wake_trace_ids: Object.freeze(duplicateTraces) }), progress: Object.freeze({ useful_progress_count: usefulProgress, heartbeat_without_useful_progress_count: heartbeatOnly }), timing: Object.freeze({ expected_wake_times: Object.freeze(times.map((value) => new Date(value).toISOString())), scheduler_clock_ms: clockDriftMs }), mismatches: Object.freeze(mismatches.map((item) => Object.freeze(item))), status: redTypesHas(mismatches, redTypes) ? 'red' : mismatches.length > 0 || heartbeatOnly > 0 || duplicateTraces.length > 0 ? 'yellow' : 'green' });
}
function redTypesHas(mismatches, redTypes) { return mismatches.some((item) => redTypes.has(item.type)); }
