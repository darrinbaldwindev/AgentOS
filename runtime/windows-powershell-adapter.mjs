...
const OPERATIONS = Object.freeze({
  ...
  'repo.diff': {
    capability: 'shell.powershell.repo.read',
    script: "git diff --no-ext-diff",
  },
  'test.run': {
    capability: 'shell.powershell.dev.execute',
    script: "npm test",
  },
  'audit.run': {
    capability: 'shell.powershell.dev.execute',
    script: "npm audit --audit-level=high --omit=dev",
  },
  'process.list': {
    capability: 'shell.powershell.system.read',
    script: "Get-Process | Select-Object -First 200 Id,ProcessName,CPU,WorkingSet64 | ConvertTo-Json -Compress",
  },
  'service.list': {
    capability: 'shell.powershell.system.read',
    script: "Get-Service | Select-Object -First 200 Name,Status,StartType | ConvertTo-Json -Compress",
  },
});
  ...
  async function execute({ operation, cwd } = {}) {
    ...
    try {
      const result = await executor({
        executable,
        args: ['-NoLogo', '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'RemoteSigned', '-Command', spec.script],
        cwd: safeCwd,
        timeoutMs,
        maxBuffer,
      });
