...
export function createGovernedExecutionBoundary({
  ...
  async function execute({ actorContext, task, invoke, actualUnits = 1 } = {}) {
    ...
    // Every gate before invoke is fail-closed. Discovery/connectivity is never authority.
    ...
    if (!riskDecision || typeof riskDecision.approvalRequired !== 'boolean') {
      throw new Error('RISK_DECISION_INVALID');
    }

    const reservation = await budget.reserve({
      project_id: task.project_id,
      mission_id: task.mission_id,
      limit_units: Math.max(1, actualUnits),
    });
    ...
    try {
      if (riskDecision.approvalRequired) {
        await approval.assertApproved({ actorContext, task, riskDecision, reservation });
      }
      ...
      const budgetResult = await budget.reconcile({
      ...
      // Do not leak reservations. Once execution starts, attempted work is charged;
      // a pre-execution approval failure reconciles at zero.
      if (!reconciled && reservation?.reservation_id) {
        try {
          await budget.reconcile({
            reservation_id: reservation.reservation_id,
            actual_units: invoked ? actualUnits : 0,
          });
        } catch (budgetError) {
          error.budgetReconciliationError = {
            name: budgetError?.name ?? 'Error',
