...
// V1 Closure Mission B: canonical requireSchedulerDisabled + mission ledger hot path.
...
export async function wakeLocal({ root, objective = 'perform one bounded local AgentOS control-cycle action', requireSchedulerDisabled = false } = {}) {
  ...
  try {
    ...
    // ONLY valid PASS reaches final COMPLETED. Mandatory pre-completion ledger.
    await appendLedgerEvent({
      root,
      stage: 'C',
      scheduleId: wakeTraceId,
