...
export function validateMissionRecord(record) {
  ...
  for (const field of ['mission_id', 'schedule_id', 'starting_repo_head', 'intended_next_action', 'outcome']) {
    requireText(record[field], field);
  }
  if (!['A', 'B', 'C'].includes(record.stage)) throw new TypeError('stage must be A, B or C');
  if (!MISSION_OUTCOMES.includes(record.outcome)) throw new TypeError(`invalid mission outcome: ${record.outcome}`);
  if (!record.timestamps || typeof record.timestamps.utc !== 'string') throw new TypeError('timestamps.utc required');
  return record;
}
