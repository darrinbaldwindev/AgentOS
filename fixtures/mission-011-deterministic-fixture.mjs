...
// Canonical worker roles from ELASTIC_WORKER_POOL.md
const WORKER_ROLES = {
  REPO_CODE: 'REPO-CODE',
  QA_TEST: 'QA-TEST',
  RESEARCH: 'RESEARCH',
...
/**
 * Create a mission with multiple tasks demonstrating DAG dependencies
 */
export function createMission011Fixture() {
  ...
  // Task DAG:
  // RESEARCH → ARCHITECTURE → REPO_CODE → QA_TEST
  //                        ↘  SECURITY_HEALTH  ↗
  const tasks = [
    ...
    createTask('task-code-002', missionId, WORKER_ROLES.REPO_CODE, ['task-arch-001'], startTime), // Parallel with code-001
    createTask('task-security-001', missionId, WORKER_ROLES.SECURITY_HEALTH, ['task-arch-001'], startTime), // Parallel
  ...
  return {
    missionId,
    tasks,
    checkpointStore: createMissionCheckpointStore(),
    startTime
  };
}
...
/**
 * Proof 1: Multiple independent tasks can progress in parallel
 * After ARCHITECTURE completes, three tasks become eligible simultaneously
 */
export function proveParallelEligibility(fixture) {
  const { tasks } = fixture;
  
  // Complete research and architecture to unblock parallel tasks
  const completedIds = new Set(['task-research-001', 'task-arch-001']);
  
  // Identify tasks that can run in parallel (all dependencies met)
...
/**
 * Proof 2: Dependency ordering is enforced
 * QA_TEST cannot start until all its dependencies complete
 */
...
/**
 * Proof 3: Concurrency and budget limits are enforced
 */
export function proveConcurrencyLimits(fixture) {
  ...
  // Simulate attempting to start 5 eligible tasks with limit of 3
  const eligibleTasks = tasks.filter(t => 
    t.task_id !== 'task-research-001' && 
    t.task_id !== 'task-test-001'
  );
  
  const allowedByConncurrency = eligibleTasks.slice(0, maxConcurrentWorkers);
  const budgetConsumed = allowedByConncurrency.length * budgetPerTask;
  const withinBudget = budgetConsumed <= totalBudget;
  
  return {
    proved: allowedByConncurrency.length === maxConcurrentWorkers && withinBudget,
    maxConcurrentWorkers,
    attemptedTasks: eligibleTasks.length,
    allowedTasks: allowedByConncurrency.length,
    budgetConsumed,
    totalBudget,
    withinBudget
  };
...
/**
 * Proof 4: Duplicate-claim prevention works
 */
...
/**
 * Proof 5: Conflict reconciliation occurs
 */
export function proveConflictReconciliation() {
  // Two workers produce conflicting results for the same task
  const evidence1 = createEvidence({
    type: 'commit',
    source: 'worker-1',
    status: 'verified',
    details: { approach: 'solution-A' }
  });
  ...
  const evidence2 = createEvidence({
    ...
    source: 'worker-2',
    status: 'verified',
    details: { approach: 'solution-B' }
  });
  
  // Mark as conflict
  const conflictEvidence = createEvidence({
    type: 'reconciliation',
    source: 'overseer',
    status: 'conflict',
    details: { worker1: evidence1, worker2: evidence2 }
  });
...
/**
 * Proof 6: Evidence is correlated to mission/task/execution
 */
export function proveEvidenceCorrelation(fixture, now = Date.now()) {
  ...
  const isCorrelated = 
    evidence.details.mission_id === missionId &&
    evidence.details.task_id === task.task_id &&
    evidence.details.execution_id === executionId;
  
  return {
    proved: isCorrelated,
    missionId: evidence.details.mission_id,
    taskId: evidence.details.task_id,
    executionId: evidence.details.execution_id,
    evidence
  };
}

/**
 * Proof 7: Recovery from last valid checkpoint succeeds
 */
export function proveCheckpointRecovery(fixture) {
  const { missionId, checkpointStore, tasks } = fixture;
  
  // Save checkpoint at task-arch-001 completion
  ...
  const checkpoint = checkpointStore.save({
    ...
    context: {
      completedTasks,
      eligibleTasks: ['task-code-001', 'task-code-002', 'task-security-001']
    },
  ...
  // Verify recovery state matches saved checkpoint
  const stateMatches = 
    recovered.missionId === checkpoint.missionId &&
    recovered.state === checkpoint.state &&
    recovered.context.completedTasks.length === completedTasks.length &&
    recovered.nextAction === checkpoint.nextAction;
  
  return {
    proved: stateMatches && recovered !== null,
    recovered: recovered !== null,
    checkpointExists: checkpoint !== null,
    stateMatches,
    recoveredContext: recovered?.context,
