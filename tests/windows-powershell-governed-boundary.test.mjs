...
function harness({ authorityAllowed = true, approvalRequired = false, approved = true } = {}) {
  ...
  const gate = (name, fn = async () => undefined) => ({
    [name]: async (input) => {
      events.push(name);
      return fn(input);
    },
  ...
  const boundary = createGovernedExecutionBoundary({
    ...
    authority: gate('assertAllowed', async () => {
      if (!authorityAllowed) throw new Error('AUTHORITY_DENIED');
    }),
    ...
    budget: {
      reserve: async () => {
        events.push('reserve');
        return { reservation_id: 'budget-1' };
      },
      reconcile: async ({ actual_units }) => {
        events.push(`reconcile:${actual_units}`);
        return { status: 'reconciled', actual_units };
      },
    },
    approval: gate('assertApproved', async () => {
      if (!approved) throw new Error('APPROVAL_DENIED');
    }),
    receipts: {
      record: async ({ result }) => {
        events.push('receipt');
        return { receipt_id: 'receipt-1', execution: result };
      },
    },
    verification: {
      verify: async ({ result, receipt }) => {
        events.push('verify');
        return { passed: result.success === true && receipt.execution === result };
      },
    },
  });
  ...
  const task = {
    ...
    mission_id: 'mission:windows-worker-test',
    task_id: 'windows-worker-test',
  };
...
test('authority denial prevents PowerShell invocation', async () => {
  ...
  await assert.rejects(
    h.boundary.execute({ actorContext: h.actorContext, task: h.task, invoke: () => h.powershell.execute({ operation: 'test.run', cwd: 'C:/agentos/AgentOS' }) }),
    /AUTHORITY_DENIED/,
  );
...
test('admitted PowerShell execution produces receipted verified evidence', async () => {
  ...
  const result = await h.boundary.execute({
    actorContext: h.actorContext,
    task: h.task,
    actualUnits: 1,
    invoke: () => h.powershell.execute({ operation: 'test.run', cwd: 'C:/agentos/AgentOS' }),
  });
