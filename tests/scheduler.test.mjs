...
test('scheduler runs a bounded number of poll cycles', async () => {
  let calls = 0;
  const scheduler = createScheduler({ poll: async () => { calls += 1; }, control: control(), intervalMs: 1, maxCycles: 3 });
...
test('scheduler can be stopped and surfaces poll errors through handler', async () => {
  let errors = 0;
  const scheduler = createScheduler({ poll: async () => { throw new Error('poll failed'); }, control: control(), intervalMs: 10, maxCycles: 5, onError: () => { errors += 1; } });
  await scheduler.start();
  scheduler.stop();
