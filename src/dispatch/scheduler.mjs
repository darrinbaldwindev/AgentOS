export function createScheduler({ poll, intervalMs = 60_000, maxCycles = Infinity, onError = () => {} }) {
  ...
  const tick = async () => {
    if (stopped || cycles >= maxCycles) return;
    cycles += 1;
    try {
      await poll();
    } catch (error) {
    ...
    if (!stopped && cycles < maxCycles) timer = setTimeout(tick, intervalMs);
  };
